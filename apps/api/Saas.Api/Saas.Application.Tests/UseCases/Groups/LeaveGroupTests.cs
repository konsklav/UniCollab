...
{
    private readonly LeaveGroup _sut;
    ...
    public LeaveGroupTests()
    {
        ...
        _sut = new LeaveGroup(_userRepository, _groupRepository);
    }
    ...
    [Fact]
    public async Task Success_WhenUserIsRemovedFromGroup()
    {
        // Arrange
        var group = FakeGroups.Generate([_user]);
        _userRepository.GetByIdAsync(_user.Id).Returns(_user);
